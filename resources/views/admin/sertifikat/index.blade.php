...
<div class="max-w-4xl">
    ...
    <div class="rounded-2xl border border-line bg-white p-5 sm:p-6"
         x-data="{ showModalPanitia: {{ $errors->has('nama_panitia') || $errors->has('nip_panitia') ? 'true' : 'false' }} }">

        <div class="flex flex-wrap items-start justify-between gap-3">
            <div>
                <h2 class="font-display text-base font-bold text-ink-900">Penandatangan Sertifikat</h2>
                <p class="mt-1 text-sm text-ink/55">Nama & NIP Ketua Panitia kegiatan terpilih, tercetak otomatis di setiap sertifikat.</p>
            </div>

            @if ($kegiatanTerpilih)
                <button type="button" @click="showModalPanitia = true"
        ...
        <div class="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
                <p class="text-[11px] font-semibold uppercase tracking-wide text-ink/40">Nama Ketua Panitia</p>
                <p class="mt-1 truncate text-sm font-semibold text-ink-900">
                    {{ optional($kegiatanTerpilih)->nama_panitia ?? '—' }}
                </p>
            </div>
            <div>
                <p class="text-[11px] font-semibold uppercase tracking-wide text-ink/40">NIP Ketua Panitia</p>
                <p class="mt-1 truncate font-mono text-sm font-semibold text-ink-900">
                    {{ optional($kegiatanTerpilih)->nip_panitia ?? '—' }}
                </p>
            </div>
        </div>

        @if (! $kegiatanTerpilih)
            <p class="mt-4 text-xs text-ink/40">Pilih kegiatan terlebih dahulu untuk melihat data panitianya.</p>
        @endif
        ...
        {{-- ===== MODAL EDIT PANITIA ===== --}}
        @if ($kegiatanTerpilih)
            <div x-show="showModalPanitia" x-cloak
                 class="fixed inset-0 z-50 flex items-center justify-center p-4"
                 style="display: none;">

                <div x-show="showModalPanitia" x-transition.opacity
                     @click="showModalPanitia = false"
                     class="absolute inset-0 bg-ink-900/40"></div>

                <div x-show="showModalPanitia"
                     ...
                     @click.outside="showModalPanitia = false"
                     class="relative w-full max-w-lg rounded-2xl border border-line bg-white p-5 shadow-lg sm:p-6">

                    <div class="flex items-center justify-between">
                        <h3 class="font-display text-base font-bold text-ink-900">Edit Ketua Panitia</h3>
                        <button type="button" @click="showModalPanitia = false"
                    ...
                    <p class="mt-1 text-xs text-ink/50">
                        Untuk kegiatan: <span class="font-semibold text-ink-900">{{ $kegiatanTerpilih->nama }}</span>
                    </p>

                    <form method="POST" action="{{ route('admin.sertifikat.panitia.update', $kegiatanTerpilih->id) }}" class="mt-5 space-y-4">
                        @csrf
                        @method('PATCH')
                        <input type="hidden" name="kegiatan_id" value="{{ $kegiatanId }}">

                        <div>
                            <label class="block text-sm font-semibold text-ink-900">Nama Ketua Panitia</label>
                            <input type="text" name="nama_panitia"
                                   value="{{ old('nama_panitia', $kegiatanTerpilih->nama_panitia) }}"
                            ...
                            @error('nama_panitia') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-ink-900">NIP Ketua Panitia</label>
                            <input type="text" name="nip_panitia" inputmode="numeric" maxlength="18"
                                   value="{{ old('nip_panitia', $kegiatanTerpilih->nip_panitia) }}"
                            ...
                            @error('nip_panitia') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                        </div>
